...
while(!bingo) {
    ...
    bingoBoards.map(function(board, index, arr) {
        arr[index] = markNumber(board, drawNumber);
        if(boardHasBingo(arr[index])) {
            bingo = true
            bingoIndex.push(index);
        }
    });
    ...
    if(bingo) {
        ...
        bingoIndex.forEach(index => {
            ...
            if(bingoBoards.length > 1) {
                bingoBoards.splice(index - amtRemoved, 1);
                amtRemoved++;
                bingo = false;
            } else {
                console.log(getScore(bingoBoards[index-amtRemoved], drawNumber));
            }
        })
    }
